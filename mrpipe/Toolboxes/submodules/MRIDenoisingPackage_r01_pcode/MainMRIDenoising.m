...
[namein, pathin, filterindex] = uigetfile({  '*.nii;*.nii.gz','NIFTI image (*.nii, *.nii.gz)'; }, 'Select one or several files (using +CTRL or +SHIFT)','MultiSelect', 'on');
if isequal(namein,0) | isequal(pathin,0)
    ...
    if (flag==1)
        ...
        for f = 1 : nbfile
            ...
            compressed = endsWith(filenamein, '.gz');
            if compressed
                % Remove the '.gz' suffix 
                filenamein = filenamein(1:end-3);
            end
            ...
            VO.Filename=outfilename;
            VO.ImageSize=s;
            VO.Datatype=class(MRIdenoised);
            niftiwrite(MRIdenoised, outfilename, VO);
