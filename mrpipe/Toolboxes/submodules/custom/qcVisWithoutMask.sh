...
INTENSITY="-a"
read -r MININT MAXINT <<< $(fslstats ${INVOL} -r)
echo "Robust MIN Intensity: ${MININT}"
echo "Robust MAX Intensity: ${MAXINT}"
...
do 
	...
	echo "Slicing: $STEP"
	slicer "$INVOL" -L -s $SYF -x $STEP $RTMP/x${i}.png
	slicer "$INVOL" -L -s $SXF -y $STEP $RTMP/y${i}.png
	slicer "$INVOL" -L -s $SXF -z $STEP $RTMP/z${i}.png
