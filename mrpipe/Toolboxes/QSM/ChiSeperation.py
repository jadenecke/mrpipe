...
class ChiSeperation(Task):
    def __init__(self, mag4d_path, pha4d_path, brainmask_path, outdir, TEms, b0_direction, CFs, Toolboxes, pre_string,
        ...
        self.command = os.path.join(
            """matlab -nosplash -nodesktop -r \"try; run('{command}'); catch ME; end; if exist('ME'); display(ME); display(ME.stack); disp(getReport(ME,'extended')); end; exit\"""")
    ...
    def getCommand(self):
        command = "Chisep_script_wResolGen(" + \
                  "'mag_path', '" + str(self.mag4d_path) + \
                  "', 'phs_path', '" + str(self.pha4d_path) + \
                  "', 'brainmask_path', '" + str(self.brainmask_path) + \
        ...
        command = self.command.format(command=command)
        return command
