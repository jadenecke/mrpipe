    ...
    class Bids(PathCollection):
        def __init__(self, filler, basepaths: PathBase, sub, ses, nameFormatter, basename):
            ...
            self.json, JsonPattern, Json_NegativePattern = Path.Identify("PET-NAV4694 json", pattern=r"[^\._]+_[^_]+_(.*)\.json",
                                                                         searchDir=self.basedir,
                                                                         previousPatterns=[nameFormatter.format(subj=sub, ses=ses, basename=pattern) + ".nii*" for pattern in PathDictPETNAV4694.getFilePatterns("JsonPattern")],
                                                                         negativePattern=[nameFormatter.format(subj=sub, ses=ses, basename=pattern) + ".nii*" for pattern in PathDictPETNAV4694.getFilePatterns("Json_NegativePattern")])
            if JsonPattern is not None:
                PathDictPETNAV4694.setFilePatterns("JsonPattern", JsonPattern)
            if Json_NegativePattern is not None:
                PathDictPETNAV4694.setFilePatterns("Json_NegativePattern", Json_NegativePattern)
