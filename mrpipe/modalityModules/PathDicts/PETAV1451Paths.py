...
class PathDictPETAV1451(PathCollection):
    ...
    class Bids(PathCollection):
        def __init__(self, filler, basepaths: PathBase, sub, ses, nameFormatter, basename):
            ...
            PETAV1451File = Path.Identify("PET-AV1451 Image", pattern=r"[^\._]+_[^_]+_(.*)\.nii.*",
                                        searchDir=self.basedir,
                                        previousPatternsName="PETAV1451Pattern",
                                        negativePatternName="PETAV1451_NegativePattern",
                                        nameFormatter=nameFormatter, sub=sub, ses=ses,
                                    fileExtensionGlob=".nii*"
                                        )

            jsonFile = Path.Identify("PET-AV1451 json", pattern=r"[^\._]+_[^_]+_(.*)\.json",
                                     searchDir=self.basedir,
                                     previousPatternsName="PETAV1451_JsonPattern",
                                     negativePatternName="PETAV1451_Json_NegativePattern",
                                     nameFormatter=nameFormatter, sub=sub, ses=ses,
