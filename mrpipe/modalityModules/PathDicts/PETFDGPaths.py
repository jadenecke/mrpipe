...
class PathDictPETFDG(PathCollection):

    class Bids(PathCollection):
        def __init__(self, filler, basepaths: PathBase, sub, ses, nameFormatter, basename):
            super().__init__(name="PETFDG_bids")
            self.basedir = Path(os.path.join(basepaths.bidsPath, filler), isDirectory=True)
            PETFDGFile, PETFDGPattern, PETFDG_NegativePattern = Path.Identify("PET-FDG Image", pattern=r"[^\._]+_[^_]+_(.*)\.nii.*",
                                                                            searchDir=self.basedir,
                                                                            previousPatterns=[nameFormatter.format(subj=sub, ses=ses, basename=pattern) + ".nii*" for pattern in PathDictPETFDG.getFilePatterns("PETFDGPattern")],
                                                                            negativePattern=[nameFormatter.format(subj=sub, ses=ses, basename=pattern) + ".nii*" for pattern in PathDictPETFDG.getFilePatterns("PETFDG_NegativePattern")])
            if PETFDGPattern is not None:
                PathDictPETFDG.setFilePatterns("PETFDGPattern", PETFDGPattern)
            if PETFDG_NegativePattern is not None:
                PathDictPETFDG.setFilePatterns("PETFDG_NegativePattern", PETFDG_NegativePattern)

            jsonFile, JsonPattern, Json_NegativePattern = Path.Identify("PET-FDG json", pattern=r"[^\._]+_[^_]+_(.*)\.json",
                                                                         searchDir=self.basedir,
                                                                         previousPatterns=[nameFormatter.format(subj=sub, ses=ses, basename=pattern) + ".json*" for pattern in PathDictPETFDG.getFilePatterns("PETFDG_JsonPattern")],
                                                                         negativePattern=[nameFormatter.format(subj=sub, ses=ses, basename=pattern) + ".json*" for pattern in PathDictPETFDG.getFilePatterns("PETFDG_Json_NegativePattern")])
            if JsonPattern is not None:
                PathDictPETFDG.setFilePatterns("PETFDG_JsonPattern", JsonPattern)
            if Json_NegativePattern is not None:
                PathDictPETFDG.setFilePatterns("PETFDG_Json_NegativePattern", Json_NegativePattern)

            self.PETFDG = ImageWithSideCar(imagePath=PETFDGFile, jsonPath=jsonFile)

    class Bids_processed(PathCollection):
        def __init__(self, filler, basepaths: PathBase, sub, ses, nameFormatter, basename):
            super().__init__(name="PETFDG_bidsProcessed")
            ...
            self.PETFDG_recentered = Path(self.basename + "_recentered.nii.gz")
            ...
            # from T1w
            self.brainstemSegmentation_inT1w = self.basename + "_brainstemSegmentation_inT1w.nii.gz"
            ...
            # SUVR calculations
            self.reMaskVal = self.basename + "_PONS_meanValue.txt"
            self.SUVR = Path(self.basename + "_PONS_SUVR.nii.gz")

            # Smoothing
            self.SUVR_smoothed4mmFWHM = Path(self.basename + "_PONS_SUVR_smoothed4mmFWHM.nii.gz")
            self.SUVR_smoothed6mmFWHM = Path(self.basename + "_PONS_SUVR_smoothed6mmFWHM.nii.gz")
            self.SUVR_smoothed8mmFWHM = Path(self.basename + "_PONS_SUVR_smoothed8mmFWHM.nii.gz")
        ...
        class Iso1p5mm(PathCollection):
            def __init__(self, filler, basepaths: PathBase, sub, ses, nameFormatter, basename):
                ...
                # To T1w
                self.baseimage = self.basename + "_PONS_SUVR_toT1w.nii.gz"
                self.SUVR_smoothed4mmFWHM_toT1w = Path(self.basename + "_PONS_SUVR_smoothed4mmFWHM_toT1w.nii.gz")
                self.SUVR_smoothed6mmFWHM_toT1w = Path(self.basename + "_PONS_SUVR_smoothed6mmFWHM_toT1w.nii.gz")
                self.SUVR_smoothed8mmFWHM_toT1w = Path(self.basename + "_PONS_SUVR_smoothed8mmFWHM_toT1w.nii.gz")

                # ToMNI
                self.toMNI = self.basename + "_PONS_SUVR_toMNI.nii.gz"
                self.SUVR_smoothed4mmFWHM_toMNI = Path(self.basename + "_PONS_SUVR_smoothed4mmFWHM_toMNI.nii.gz")
                self.SUVR_smoothed6mmFWHM_toMNI = Path(self.basename + "_PONS_SUVR_smoothed6mmFWHM_toMNI.nii.gz")
                self.SUVR_smoothed8mmFWHM_toMNI = Path(self.basename + "_PONS_SUVR_smoothed8mmFWHM_toMNI.nii.gz")
    ...
    class Meta_QC(PathCollection):
        def __init__(self, filler, basepaths: PathBase, sub, ses, nameFormatter, basename):
            ...
            self.ToT1w_native_slices = self.basename + "_PETFDGToT1w_native.png"
            self.refMask_native_slices = self.basename + "_PETFDG_refMask.png"
    ...
    def __init__(self, sub, ses, basepaths, basedir="pet-FDG", nameFormatter="{subj}_{ses}_{basename}",
                 modalityBeforeSession=False, basename="pet-FDG"):
        super().__init__(name="PETFDG")
