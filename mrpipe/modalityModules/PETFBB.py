...
class PETFBB_base_withT1w(ProcessingModule):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.petfbb_base_fromT1w_schaefer100_7Net = PipeJobPartial(name="PETFBB_base_fromT1w_schaefer100_7Net", job=SchedulerPartial(
            taskList=[AntsApplyTransforms(input=session.subjectPaths.T1w.bids_processed.Schaefer2018_100Parcels_7Networks_order_FSLMNI152_gmMasked,
                                          output=session.subjectPaths.pet_fbb.bids_processed.atlas_schaefer100_7Net,
                                          reference=session.subjectPaths.pet_fbb.bids_processed.PETFBB_recentered,
                                          transforms=[session.subjectPaths.pet_fbb.bids_processed.toT1w_0GenericAffine],
                                          inverse_transform=[True],
                                          interpolation="NearestNeighbor",
        ...
        self.petfbb_base_qc_vis_RefRegion = PipeJobPartial(name="PETFBB_base_slices_RefRegion", job=SchedulerPartial(
            taskList=[QCVis(infile=session.subjectPaths.pet_fbb.bids_processed.PETFBB_recentered,
                            mask=session.subjectPaths.pet_fbb.bids_processed.refMask,
                            image=session.subjectPaths.pet_fbb.meta_QC.refMask_native_slices, contrastAdjustment=True,
                            outline=True, transparency=True, zoom=1,
                            session=session) for session in self.sessions]), env=self.envs.envQCVis)
        ...
        self.petfbb_base_suvr_statsSchaefer100_7 = PipeJobPartial(name="PETFBB_base_SUVR_statsSchaefer100_7", job=SchedulerPartial(
            taskList=[ExtractAtlasValues(infile=session.subjectPaths.pet_fbb.bids_processed.SUVR,
                                         atlas=session.subjectPaths.pet_fbb.bids_processed.atlas_schaefer100_7Net,
                                         outfile=session.subjectPaths.pet_fbb.bids_statistics.SUVR_WHOLECER_Schaefer100_7Net_mean,
                                         func="mean",
                                         session=session) for session in self.sessions]), env=self.envs.envR)
        ...
        self.petfbb_base_suvrToCentiloid_Schaefer100_7 = PipeJobPartial(name="PETFBB_base_SUVRToCentiloid_Schaefer100_7", job=SchedulerPartial(
            taskList=[SUVRToCentiloid(infile=session.subjectPaths.pet_fbb.bids_statistics.SUVR_WHOLECER_Schaefer100_7Net_mean,
                                      outfile=session.subjectPaths.pet_fbb.bids_statistics.Centiloid_WHOLECER_Schaefer100_7Net_mean,
                                      tracerName="FBB",
                                      session=session) for session in self.sessions]), env=self.envs.envR)
