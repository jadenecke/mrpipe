...
class PETPI2620_base_withT1w(ProcessingModule):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.petpi2620_base_fromT1w_schaefer100_7Net = PipeJobPartial(name="PETPI2620_base_fromT1w_schaefer100_7Net", job=SchedulerPartial(
            taskList=[AntsApplyTransforms(input=session.subjectPaths.T1w.bids_processed.Schaefer2018_100Parcels_7Networks_order_FSLMNI152_gmMasked,
                                          output=session.subjectPaths.pet_pi2620.bids_processed.atlas_schaefer100_7Net,
                                          reference=session.subjectPaths.pet_pi2620.bids_processed.PETPI2620_recentered,
                                          transforms=[session.subjectPaths.pet_pi2620.bids_processed.toT1w_0GenericAffine],
                                          inverse_transform=[True],
                                          interpolation="NearestNeighbor",
        ...
        self.petpi2620_base_qc_vis_RefRegion = PipeJobPartial(name="PETPI2620_base_slices_RefRegion", job=SchedulerPartial(
            taskList=[QCVis(infile=session.subjectPaths.pet_pi2620.bids_processed.PETPI2620_recentered,
                            mask=session.subjectPaths.pet_pi2620.bids_processed.refMask,
                            image=session.subjectPaths.pet_pi2620.meta_QC.refMask_native_slices, contrastAdjustment=True,
                            outline=True, transparency=True, zoom=1,
                            session=session) for session in self.sessions]), env=self.envs.envQCVis)
        ...
        self.petpi2620_base_suvr_statsSchaefer100_7 = PipeJobPartial(name="PETPI2620_base_SUVR_statsSchaefer100_7", job=SchedulerPartial(
            taskList=[ExtractAtlasValues(infile=session.subjectPaths.pet_pi2620.bids_processed.SUVR,
                                         atlas=session.subjectPaths.pet_pi2620.bids_processed.atlas_schaefer100_7Net,
                                         outfile=session.subjectPaths.pet_pi2620.bids_statistics.SUVR_INFCER_Schaefer100_7Net_mean,
                                         func="mean",
                                         session=session) for session in self.sessions]), env=self.envs.envR)
...
class PETPI2620_native_CenTauRZ(ProcessingModule):
    requiredModalities = ["T1w", "pet_pi2620"]
    moduleDependencies = ["T1w_SynthSeg", "PETPI2620_base_withT1w"]
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.petpi2620_centaurz_stats_CenTauR = PipeJobPartial(name="PETPI2620_centaurz_stats_CenTauR", job=SchedulerPartial(
            taskList=[FSLStats(infile=session.subjectPaths.pet_pi2620.bids_processed.SUVR,
                               ...
                               options=["-k", "-M"],
                               mask=session.subjectPaths.pet_pi2620.bids_processed.centaur_maskNative_CenTauR,
        ...
        self.petpi2620_centaurz_fromMNI_Frontal_CenTauR = PipeJobPartial(name="PETPI2620_centaurz_fromMNI_Frontal_CenTauR", job=SchedulerPartial(
            taskList=[CAT12_WarpToTemplate(infile=self.templates.centaur_Frontal_CenTauR,
                                           tempdir=self.basepaths.scratch,
                                           outfile=session.subjectPaths.pet_pi2620.bids_processed.centaur_maskNative_Frontal_CenTauR_inT1w,
                                           warpfile=session.subjectPaths.T1w.bids_processed.cat12.cat12_T1ToMNI_InverseWarp,
                                           interp=ValidCat12Interps.nearestNeighbor,
                                           voxelsize=1,
                                           session=session) for session in self.sessions], cpusPerTask=3), env=self.envs.envSPM12)
        ...
        self.petpi2620_centaurz_fromT1w_Frontal_CenTauR = PipeJobPartial(name="PETPI2620_centaurz_fromT1w_Frontal_CenTauR", job=SchedulerPartial(
            taskList=[AntsApplyTransforms(input=session.subjectPaths.pet_pi2620.bids_processed.centaur_maskNative_Frontal_CenTauR_inT1w,
        ...
        self.petpi2620_centaurz_stats_Frontal_CenTauR = PipeJobPartial(name="PETPI2620_centaurz_stats_Frontal_CenTauR", job=SchedulerPartial(
            taskList=[FSLStats(infile=session.subjectPaths.pet_pi2620.bids_processed.SUVR,
                               ...
                               options=["-k", "-M"],
                               mask=session.subjectPaths.pet_pi2620.bids_processed.centaur_maskNative_Frontal_CenTauR,
                               session=session) for session in self.sessions], cpusPerTask=3), env=self.envs.envFSL)
        ...
        self.petpi2620_centaurz_fromMNI_Mesial_CenTauR = PipeJobPartial(name="PETPI2620_centaurz_fromMNI_Mesial_CenTauR", job=SchedulerPartial(
            taskList=[CAT12_WarpToTemplate(infile=self.templates.centaur_Mesial_CenTauR,
                                           tempdir=self.basepaths.scratch,
                                           outfile=session.subjectPaths.pet_pi2620.bids_processed.centaur_maskNative_Mesial_CenTauR_inT1w,
                                           warpfile=session.subjectPaths.T1w.bids_processed.cat12.cat12_T1ToMNI_InverseWarp,
                                           interp=ValidCat12Interps.nearestNeighbor,
                                           voxelsize=1,
                                           session=session) for session in self.sessions], cpusPerTask=3), env=self.envs.envSPM12)

        self.petpi2620_centaurz_fromT1w_Mesial_CenTauR = PipeJobPartial(name="PETPI2620_centaurz_fromT1w_Mesial_CenTauR", job=SchedulerPartial(
            taskList=[AntsApplyTransforms(input=session.subjectPaths.pet_pi2620.bids_processed.centaur_maskNative_Mesial_CenTauR_inT1w,
        ...
        self.petpi2620_centaurz_stats_Mesial_CenTauR = PipeJobPartial(name="PETPI2620_centaurz_stats_Mesial_CenTauR", job=SchedulerPartial(
            taskList=[FSLStats(infile=session.subjectPaths.pet_pi2620.bids_processed.SUVR,
                               ...
                               options=["-k", "-M"],
                               mask=session.subjectPaths.pet_pi2620.bids_processed.centaur_maskNative_Mesial_CenTauR,
                               session=session) for session in self.sessions], cpusPerTask=3), env=self.envs.envFSL)
        ...
        self.petpi2620_centaurz_fromMNI_Meta_CenTauR = PipeJobPartial(name="PETPI2620_centaurz_fromMNI_Meta_CenTauR", job=SchedulerPartial(
            taskList=[CAT12_WarpToTemplate(infile=self.templates.centaur_Meta_CenTauR,
                                           tempdir=self.basepaths.scratch,
                                           outfile=session.subjectPaths.pet_pi2620.bids_processed.centaur_maskNative_Meta_CenTauR_inT1w,
                                           warpfile=session.subjectPaths.T1w.bids_processed.cat12.cat12_T1ToMNI_InverseWarp,
                                           interp=ValidCat12Interps.nearestNeighbor,
                                           voxelsize=1,
                                           session=session) for session in self.sessions], cpusPerTask=3), env=self.envs.envSPM12)

        self.petpi2620_centaurz_fromT1w_Meta_CenTauR = PipeJobPartial(name="PETPI2620_centaurz_fromT1w_Meta_CenTauR", job=SchedulerPartial(
            taskList=[AntsApplyTransforms(input=session.subjectPaths.pet_pi2620.bids_processed.centaur_maskNative_Meta_CenTauR_inT1w,
        ...
        self.petpi2620_centaurz_stats_Meta_CenTauR = PipeJobPartial(name="PETPI2620_centaurz_stats_Meta_CenTauR", job=SchedulerPartial(
            taskList=[FSLStats(infile=session.subjectPaths.pet_pi2620.bids_processed.SUVR,
                               ...
                               options=["-k", "-M"],
                               mask=session.subjectPaths.pet_pi2620.bids_processed.centaur_maskNative_Meta_CenTauR,
                               session=session) for session in self.sessions], cpusPerTask=3), env=self.envs.envFSL)
        ...
        self.petpi2620_centaurz_fromMNI_TP_CenTauR = PipeJobPartial(name="PETPI2620_centaurz_fromMNI_TP_CenTauR", job=SchedulerPartial(
            taskList=[CAT12_WarpToTemplate(infile=self.templates.centaur_TP_CenTauR,
                                           tempdir=self.basepaths.scratch,
                                           outfile=session.subjectPaths.pet_pi2620.bids_processed.centaur_maskNative_TP_CenTauR_inT1w,
                                           warpfile=session.subjectPaths.T1w.bids_processed.cat12.cat12_T1ToMNI_InverseWarp,
                                           interp=ValidCat12Interps.nearestNeighbor,
                                           voxelsize=1,
                                           session=session) for session in self.sessions], cpusPerTask=3), env=self.envs.envSPM12)

        self.petpi2620_centaurz_fromT1w_TP_CenTauR = PipeJobPartial(name="PETPI2620_centaurz_fromT1w_TP_CenTauR", job=SchedulerPartial(
            taskList=[AntsApplyTransforms(input=session.subjectPaths.pet_pi2620.bids_processed.centaur_maskNative_TP_CenTauR_inT1w,
        ...
        self.petpi2620_centaurz_stats_TP_CenTauR = PipeJobPartial(name="PETPI2620_centaurz_stats_TP_CenTauR", job=SchedulerPartial(
            taskList=[FSLStats(infile=session.subjectPaths.pet_pi2620.bids_processed.SUVR,
                               ...
                               options=["-k", "-M"],
                               mask=session.subjectPaths.pet_pi2620.bids_processed.centaur_maskNative_TP_CenTauR,
                               session=session) for session in self.sessions], cpusPerTask=3), env=self.envs.envFSL)
